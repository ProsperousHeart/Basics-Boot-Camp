# Additional Links & Resources

## About This Lesson

Explore additional Python resources and learning materials. Find documentation, tutorials, communities, and tools to continue your Python journey.

### What You'll Find
- Official Python documentation
- Online learning resources
- Python communities and forums
- Useful Python tools and libraries
- Best practices and style guides
- Advanced topics to explore next

### Navigation Tips
- Use the MkDocs navigation on the left to move between lessons
- The notebook below is fully interactive - you can run and edit code
- Your changes are saved in your browser's local storage

---

## Interactive Notebook

<div style="width: 100%; height: 800px; border: 1px solid #ddd; border-radius: 4px; overflow: hidden;">
  <iframe
    src="../../../jupyterlite/notebooks/index.html?path=BC_Weeks/Week_3/Python_Basics_19_-_Additional_Links.ipynb"
    width="100%"
    height="100%"
    frameborder="0"
    allow="cross-origin-isolated"
    style="border: none;">
  </iframe>
</div>

---

**Having trouble with the embedded notebook?** Try one of these:
- [Open in Notebooks view](../../../jupyterlite/notebooks/index.html?path=BC_Weeks/Week_3/Python_Basics_19_-_Additional_Links.ipynb){:target="_blank"}
- [Open in full JupyterLab](../../../jupyterlite/lab/index.html?path=BC_Weeks/Week_3/Python_Basics_19_-_Additional_Links.ipynb){:target="_blank"}
